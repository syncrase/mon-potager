...
public class PlanteRepositoryWithBagRelationshipsImpl implements PlanteRepositoryWithBagRelationships {
    ...
    public Optional<Plante> fetchBagRelationships(Optional<Plante> plante) {
        return plante.map(this::fetchNomsVernaculaires).map(this::fetchReferences);
    }
    ...
    public List<Plante> fetchBagRelationships(List<Plante> plantes) {
        return Optional.of(plantes).map(this::fetchNomsVernaculaires).map(this::fetchReferences).orElse(Collections.emptyList());
    }
    ...
    Plante fetchReferences(Plante result) {
        return entityManager
            .createQuery("select plante from Plante plante left join fetch plante.references where plante is :plante", Plante.class)
            .setParameter("plante", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Plante> fetchReferences(List<Plante> plantes) {
        return entityManager
            .createQuery(
                "select distinct plante from Plante plante left join fetch plante.references where plante in :plantes",
                Plante.class
            )
            .setParameter("plantes", plantes)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
