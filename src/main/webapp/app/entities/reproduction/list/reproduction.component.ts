...
export class ReproductionComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.reproductionService
      ...
      .subscribe({
        next: (res: HttpResponse<IReproduction[]>) => {
  ...
  trackId(_index: number, item: IReproduction): number {
    return item.id!;
  }

  delete(reproduction: IReproduction): void {
    const modalRef = this.modalService.open(ReproductionDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IReproduction[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/reproduction'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
