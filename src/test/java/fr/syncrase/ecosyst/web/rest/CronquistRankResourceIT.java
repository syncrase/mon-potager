...
class CronquistRankResourceIT {
    ...
    public static CronquistRank createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, NomVernaculaire.class).isEmpty()) {
            nomVernaculaire = NomVernaculaireResourceIT.createEntity(em);
            em.persist(nomVernaculaire);
            em.flush();
        } else {
            nomVernaculaire = TestUtil.findAll(em, NomVernaculaire.class).get(0);
        }
        cronquistRank.getNoms().add(nomVernaculaire);
    ...
    public static CronquistRank createUpdatedEntity(EntityManager em) {
        ...
        NomVernaculaire nomVernaculaire;
        if (TestUtil.findAll(em, NomVernaculaire.class).isEmpty()) {
            nomVernaculaire = NomVernaculaireResourceIT.createUpdatedEntity(em);
            em.persist(nomVernaculaire);
            em.flush();
        } else {
            nomVernaculaire = TestUtil.findAll(em, NomVernaculaire.class).get(0);
    ...
    void getAllCronquistRanksByNomsIsEqualToSomething() throws Exception {
        // Initialize the database
        cronquistRankRepository.saveAndFlush(cronquistRank);
        NomVernaculaire noms;
        if (TestUtil.findAll(em, NomVernaculaire.class).isEmpty()) {
            noms = NomVernaculaireResourceIT.createEntity(em);
            em.persist(noms);
            em.flush();
        } else {
            noms = TestUtil.findAll(em, NomVernaculaire.class).get(0);
        }
        em.persist(noms);
        em.flush();
