...
class CronquistRankResourceIT {
    ...
    private static final CronquistTaxonomicRank DEFAULT_RANK = CronquistTaxonomicRank.DOMAINE;
    private static final CronquistTaxonomicRank UPDATED_RANK = CronquistTaxonomicRank.SOUSDOMAINE;
    ...
    @Test
    @Transactional
    void getAllCronquistRanksByClassificationIsEqualToSomething() throws Exception {
        // Initialize the database
        cronquistRankRepository.saveAndFlush(cronquistRank);
        ...
        em.persist(classification);
        em.flush();
        cronquistRank.setClassification(classification);
        cronquistRankRepository.saveAndFlush(cronquistRank);
        Long classificationId = classification.getId();

        // Get all the cronquistRankList where classification equals to classificationId
        defaultCronquistRankShouldBeFound("classificationId.equals=" + classificationId);

        // Get all the cronquistRankList where classification equals to (classificationId + 1)
        defaultCronquistRankShouldNotBeFound("classificationId.equals=" + (classificationId + 1));
    }
