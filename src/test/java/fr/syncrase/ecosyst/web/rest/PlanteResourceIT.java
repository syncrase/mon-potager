...
class PlanteResourceIT {
    ...
    @Test
    @Transactional
    void getAllPlantesByLowestClassificationRankIsEqualToSomething() throws Exception {
        // Initialize the database
        planteRepository.saveAndFlush(plante);
        CronquistRank lowestClassificationRank;
        if (TestUtil.findAll(em, CronquistRank.class).isEmpty()) {
            lowestClassificationRank = CronquistRankResourceIT.createEntity(em);
            em.persist(lowestClassificationRank);
            em.flush();
        } else {
            lowestClassificationRank = TestUtil.findAll(em, CronquistRank.class).get(0);
        }
        em.persist(lowestClassificationRank);
        em.flush();
        plante.addLowestClassificationRank(lowestClassificationRank);
        planteRepository.saveAndFlush(plante);
        Long lowestClassificationRankId = lowestClassificationRank.getId();

        // Get all the planteList where lowestClassificationRank equals to lowestClassificationRankId
        defaultPlanteShouldBeFound("lowestClassificationRankId.equals=" + lowestClassificationRankId);

        // Get all the planteList where lowestClassificationRank equals to (lowestClassificationRankId + 1)
        defaultPlanteShouldNotBeFound("lowestClassificationRankId.equals=" + (lowestClassificationRankId + 1));
    }
