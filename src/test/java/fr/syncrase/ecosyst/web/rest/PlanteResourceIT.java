...
class PlanteResourceIT {
    ...
    @Test
    @Transactional
    void getAllPlantesByCronquistRankIsEqualToSomething() throws Exception {
        // Initialize the database
        planteRepository.saveAndFlush(plante);
        ...
        em.persist(cronquistRank);
        em.flush();
        plante.setCronquistRank(cronquistRank);
        planteRepository.saveAndFlush(plante);
        Long cronquistRankId = cronquistRank.getId();

        // Get all the planteList where cronquistRank equals to cronquistRankId
        defaultPlanteShouldBeFound("cronquistRankId.equals=" + cronquistRankId);

        // Get all the planteList where cronquistRank equals to (cronquistRankId + 1)
        defaultPlanteShouldNotBeFound("cronquistRankId.equals=" + (cronquistRankId + 1));
    }
