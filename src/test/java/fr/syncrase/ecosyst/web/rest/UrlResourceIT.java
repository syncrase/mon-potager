...
class UrlResourceIT {
    ...
    @Test
    @Transactional
    void getAllUrlsByReferencesIsEqualToSomething() throws Exception {
        // Initialize the database
        urlRepository.saveAndFlush(url);
        ...
        em.persist(references);
        em.flush();
        url.addReferences(references);
        urlRepository.saveAndFlush(url);
        Long referencesId = references.getId();

        // Get all the urlList where references equals to referencesId
        defaultUrlShouldBeFound("referencesId.equals=" + referencesId);

        // Get all the urlList where references equals to (referencesId + 1)
        defaultUrlShouldNotBeFound("referencesId.equals=" + (referencesId + 1));
    }
